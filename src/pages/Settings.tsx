...
import { Building2, Upload, Trash2, ShieldAlert, Check, Sliders, ArrowRight } from 'lucide-react';
...
export default function Settings() {
  ...
  const activeTab = searchParams.get('tab') === 'accessibility' ? 'accessibility' : 'general';
  ...
  const { data: profile, isLoading } = useQuery<Profile>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, email, agency_name, logo_url, plan, white_label_enabled, weekly_digest_enabled, review_interval_days, created_at')
    ...
    <div className="max-w-4xl space-y-8">
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-muted text-sm tracking-[0.15em]">MANAGE YOUR AGENCY PROFILE, PREFERENCES, AND ACCESSIBILITY TOOLS.</p>
        </div>

        {/* Navigation Tabs */}
        <div className="flex border-b border-white/10 space-x-6 text-xs uppercase tracking-widest pt-2">
          <button
            onClick={() => setSearchParams({ tab: 'general' })}
            className={`pb-4 font-bold border-b-2 transition-colors ${
              activeTab === 'general' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-white'
            }`}
          >
            General Preferences
          </button>
          <button
            onClick={() => setSearchParams({ tab: 'accessibility' })}
            className={`pb-4 font-bold border-b-2 transition-colors flex items-center gap-2 ${
              activeTab === 'accessibility' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-white'
            }`}
          >
        ...
        <div className="space-y-12">
          ...
          <div className="bg-surface border border-white/10 p-10 space-y-10">
            ...
            <div className="space-y-8">
              ...
              {/* Accessibility Tools Link Card */}
              <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                <div className="space-y-1">
                  <h4 className="font-bold text-sm tracking-wider">ACCESSIBILITY & COMPLIANCE TOOLS</h4>
                  <p className="text-xs text-muted font-light">MANAGE WCAG 2.1 AA WIDGET EMBEDS AND VPAT 2.4 CONFORMANCE STATEMENTS.</p>
                </div>
                <button
                  onClick={() => setSearchParams({ tab: 'accessibility' })}
                  className="bracket-btn py-2 px-5 text-xs font-black uppercase tracking-wider flex items-center gap-2"
                >
                  <span className="bracket-btn-inner"></span>
                  OPEN TOOLS <ArrowRight className="w-3.5 h-3.5 text-accent" />
                </button>
              </div>
            </div>
          </div>
